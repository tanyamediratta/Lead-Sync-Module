...
export default function App() {
  ...
  async function syncAll() {
    ...
    try {
      ...
      const imported =
        (data?.meta?.imported ?? 0) + (data?.google?.imported ?? 0);
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full text-sm">
                ...
                <tbody>
                  {leads.map((l) => (
                    <tr
                      key={`${l.email}-${l.platform}-${l.timestamp}`}
                    ...
                    >
                      ...
                      <td className="py-2 pr-4">{l.campaign || "-"}</td>
                      <td className="py-2 pr-4">{fmt(l.timestamp)}</td>
                    </tr>
                  ))}
                  {leads.length === 0 && (
                    <tr>
          ...
          {!loadingLogs ? (
            logs.length ? (
              <ul className="space-y-2 text-sm">
                {logs.map((log) => (
                  <li
                    key={`${log.timestamp}-${log.source}-${log.status}`}
                  ...
                  >
                    ...
                    <span className="text-gray-600">
                      fetched: {log.fetched ?? 0} • imported: {log.imported ?? 0}
                    </span>
